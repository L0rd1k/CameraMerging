...
vector<Point2f> PointsCollectorChess::collectFramePoints(Mat &image) {
    ...
    for(double p = 3; p <= 8; p++) { // scale image until chessboard would not be detected
        Mat imageResized;
        double scale = pow(1.2, p);
        resize(image, imageResized, Size(), scale, scale, INTER_CUBIC);
        ...
        if(chessboardResult) { // if chessboard corners were detecting
            cornerSubPix(imageResized, pts, Size(11,11), Size(-1,-1), TermCriteria(CV_TERMCRIT_EPS + CV_TERMCRIT_ITER, 50, 0.01));
            for(auto &p: pts) {
                p.x /= scale;
                p.y /= scale;
            }
