...
Rect centerRoi(Size large, Size size) {
    Rect r;
    r.x = (large.width - size.width) / 2;
    r.y = (large.height - size.height) / 2;
    r.width = size.width;
...
Mat DualCameraMerger::merge(Mat img1, Mat img2) { // img1- TV img-IK 
    ...
    Size largeSize(w, h); // take the size of the largest side

    Mat largerImage(largeSize, edges.type(), Scalar(0));

    Mat large1(largeSize, img1.type(), Scalar(0));
    Mat large2(largeSize, img2.type(), Scalar(0));
    Mat roi1(large1, Rect(0, 0, img1.cols, img1.rows));
    Mat roi2(large2, Rect(0, 0, img2.cols, img2.rows));
    img1.copyTo(roi1);
    img2.copyTo(roi2);

    Mat tmp = large1.clone();
    GaussianBlur(tmp, tmp, Size(7, 7), 1.5, 1.5);
    Canny(tmp, edges, 0, 30, 3);
    ...
    ////    Mat roi1(large1, centerRoi(largeSize, img1.size()));
    ////    Mat roi2(large2, centerRoi(largeSize, img2.size()));
    //    Mat roi1(large1, Rect(0, 0, img1.cols, img1.rows));
    //    Mat roi2(large2, Rect(0, 0, img2.cols, img2.rows));
    //    img1.copyTo(roi1);
    //    img2.copyTo(roi2);
